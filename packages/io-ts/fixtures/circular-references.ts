...
export const Teacher: Type<_Teacher> = recursion('Teacher', () =>
  intersection([
    type({ type: literal('teacher') }),
    partial({
      students: union([array(Student), Undefined]),
      reportsTo: union([Teacher, Undefined]),
    }),
  ])
)
...
export const Student: Type<_Student> = recursion('Student', () =>
  type({ type: literal('student'), teacher: Teacher })
)
