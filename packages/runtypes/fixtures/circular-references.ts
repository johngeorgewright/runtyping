...
export const Teacher: Runtype<_Teacher> = Lazy(() =>
  Record({
    type: Literal('teacher'),
    students: Array(Student).Or(Undefined).optional(),
    reportsTo: Teacher.Or(Undefined).optional(),
  })
)

export type Teacher = Static<typeof Teacher>

export const Student: Runtype<_Student> = Lazy(() =>
  Record({ type: Literal('student'), teacher: Teacher })
)
