...
export const A = Array(Unknown).withConstraint<_A>(data =>
  data.length >= 1
  && String.guard(data[0])
  && Array(String).guard(data.slice(1, undefined))
);
...
export const B = Array(Unknown).withConstraint<_B>(data =>
  ...
  && String.guard(data[0])
  && Number.guard(data[1])
  && Array(Number).guard(data.slice(2, undefined))
);
...
export const C = Array(Unknown).withConstraint<_C>(data =>
  data.length >= 1
  && Array(String).guard(data.slice(0, -1))
  && Number.guard(data.slice(-1)[0])
);
...
export const D = Array(Unknown).withConstraint<_D>(data =>
  data.length >= 2
  && String.guard(data[0])
  && Array(String).guard(data.slice(1, -1))
  && String.guard(data.slice(-1)[0])
);
...
export const E = Array(Unknown).withConstraint<_E>(data =>
  ...
  && String.guard(data[0])
  && Number.guard(data[1])
  && Boolean.guard(data[2])
  && Array(String).guard(data.slice(3, -2))
  && Number.guard(data.slice(-2)[0])
  && Boolean.guard(data.slice(-1)[0])
);
